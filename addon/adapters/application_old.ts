...
import ParseObject from 'dummy/tests/dummy/app/models/parseObject';
...
export default DS.RESTAdapter.extend({
  ...
  createRecord<k extends never>(store: DS.Store, type: ModelRegistry[k], record: DS.Snapshot<k>): RSVP.Promise<any> {
    ...
    return new RSVP.Promise(
      ...
      {
          let saveObject = Parse.Object.extend(this.parseClassForType(record.modelName))
          let entries = Object.entries(data)
          let attributes = entries
            .filter(([key, value]) => (!(typeof value === 'object')))
            .map(([key, value]) => {
              saveObject.set(key, value)
            })
          saveObject = await saveObject.save()
          resolve(saveObject.toJSON)
      }
    );
